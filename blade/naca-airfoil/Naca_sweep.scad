...
module help()
{
  ...
  "module sweep(dat, convexity = 5, showslices= false) // dat - vec of vec2, with vec1 = airfoil_data\n", 
...
// dat - vec of vec1, with vec1 = airfoil_data, >3 points per dataset expected
module sweep(dat, convexity = 5, showslices= false) 
{
  n = len(dat);     // # datasets
  ...
  {
    first = faces_polygon(l, true);  // triangulate first dataset
    last = faces_shift((n-2)*l, faces_polygon(l, false)); // triangulate last dataset
    if (showslices)
      for(i=[0:n-1])
        sweep([dat[i]]); 
    else
...
function Rx_(x=0, v) = [for(i = [0:len(v)-1])
  let (yz = rot(x, p=[v[i][1], v[i][2]])) [v[i][0], yz[0], yz[1]]];  

function Ry_(y=0, v) = [for(i = [0:len(v)-1])
  let (xz = rot(y, p=[v[i][0], v[i][2]])) [xz[0], v[i][1], xz[1]]];  
function Rz_(z=0, v) = [for (i=[0:len(v)-1]) // 2D vectors allowed  
  let (xy = rot(z, p=[v[i][0], v[i][1]])) 
  len(v[0])==3?[xy[0], xy[1], v[i][2]]:xy];  
// single point in 2D
function rot(w=0, p) = [p[0]*cos(w)-p[1]*sin(w), p[0]*sin(w)+p[1]*cos(w)]; 
